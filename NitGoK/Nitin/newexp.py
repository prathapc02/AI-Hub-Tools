...
# Function to expand window and switch between frames
def expand_window(content_frame, title):
    global current_frame
    if current_frame:
        current_frame.destroy()

    current_frame = Frame(content_frame, width=1000, height=700, bg='#000000')  
...
def display_home(frame):
    ...
    panel = Label(frame, image=img, text="AI Tools GUI", compound="top", fg="white", bg="#000000")  
    panel.image = img
    panel.pack(expand=True)

# Function to display AI tools content
def display_ai_tools(frame):
    notebook = ttk.Notebook(frame)
    notebook.pack(expand=True, fill='both')

    tab_info = [
        {"name": "Face Dataset Creation|", "image_path": "fdc.jpg", "button_text": "START CAPTURE", "command": capture_faces},
    ...
    for i, tab_info in enumerate(tab_info):
        ...
        notebook.add(tab_frame, text=tab_info["name"])
        ...
        button = Button(tab_frame, text=tab_info["button_text"], width=20, height=2, bg="white", fg="#262626",
                        command=tab_info.get("command"))
        button.pack(pady=6)
...
def display_necessary_tools(frame):
    ...
    image_commands = [
        ...
        ("1.jpg", "Button 2", command2),
        #("1.jpg", "Button 3", command3),
        ("1.jpg", "Button 4", command4),
        ("1.jpg", "Button 5", command5),
        ("1.jpg", "Button 6", command6),
        ("1.jpg", "Button 7", command7),
        ("1.jpg", "Button 8", command8),
        ("1.jpg", "Button 9", command9),
        ("1.jpg", "Button 10", command10)
    ]
    ...
    for i, (image_path, label_text, command) in enumerate(image_commands):
        ...
        btn = Button(frame, image=img, command=command, bg="#808080", fg="#262626", relief=FLAT)
        btn.image = img
        btn.grid(row=i // 5, column=i % 5, padx=5, pady=5, sticky="nsew")

        label = Label(frame, text=label_text, bg="#000000", fg="white")
...
def command9():
    subprocess.Popen(['python', '06_camera.py'])
...
def display_about(frame):
    label = Label(frame, text="Author: Your Name\nAddress: Your Address", font=("Arial", 16), fg="white", bg="#000000")  
